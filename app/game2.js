...
function create() {
	...
	bad_objects = createGroup(100, game.cache.getBitmapData('bad'));

	slashes = game.add.graphics(0, 0);
	

	scoreLabel = game.add.text(20,20,'Recoge 50 bolas complementarias al naranja');
    ...
    buttonText = game.add.text(nextLevelButton.x, nextLevelButton.y, 'Siguiente Nivel', { font: 'Arial', fontSize: '16px', fill: '#ffffff', margin: '30px' });
...
function throwBadObject() {
    ...
    if (obj) { 
        obj.reset(game.world.centerX + Math.random() * 600 - Math.random() * 100, 600);
        obj.anchor.setTo(3, 3);
        var randomColor = Phaser.Color.getRandomColor(50, 255, 255);
        obj.tint = randomColor;
        game.physics.arcade.moveToXY(obj, game.world.centerX, game.world.centerY, 1000);
    }
}
...
function showCongratulationMessage() {
    var congratulationText = game.add.text(game.world.centerX, game.world.centerY, 'ENORABUENA! YA SABES LOS COLORES COMPLEMENTARIOS', { font: 'bold 60px Arial', fill: '#FF9100' });
